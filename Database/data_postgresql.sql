-- data_postgresql.sql
-- PostgreSQL version of data generation script

-- Note: Run this AFTER creating the schema (schema_postgresql.sql)
...
WITH RECURSIVE 
years AS (
    SELECT 1 as year_num, 'L1' as annee 
    UNION SELECT 2, 'L2' 
    UNION SELECT 3, 'L3' 
    UNION SELECT 4, 'M1' 
    UNION SELECT 5, 'M2'
...
modules_per_year AS (
    SELECT 
        ...
        -- PostgreSQL: FLOOR(6 + RANDOM() * 4)
        FLOOR(6 + RANDOM() * 4)::INTEGER as num_modules
    FROM formations f
    CROSS JOIN years y
...
SELECT 
    -- PostgreSQL concatenation: 'string' || variable || 'string'
    'MOD_' || mp.annee || '_F' || mp.id_formation || '_' || mn.n,
    5,
    mp.id_formation,
    mp.annee
...
WITH 
group_nums_L AS (SELECT generate_series(1, 5) as n),
group_nums_M AS (SELECT generate_series(1, 3) as n),
years_L AS (SELECT unnest(ARRAY['L1', 'L2', 'L3']) as annee),
years_M AS (SELECT unnest(ARRAY['M1', 'M2']) as annee)
-- Licence Groups
SELECT 
    -- PostgreSQL: use LPAD or to_char
    f.id_formation || '-' || y.annee || '-G' || LPAD(gn.n::TEXT, 2, '0'),
    f.id_formation,
    y.annee,
    30
FROM formations f
...
students_per_group AS (
    SELECT 1 as n 
    UNION ALL 
    SELECT n + 1 FROM students_per_group WHERE n < 30
)
SELECT 
    'Nom_' || g.code_groupe || '_' || LPAD(spg.n::TEXT, 2, '0'),
    'Prenom_' || LPAD(FLOOR(RANDOM() * 1000)::TEXT, 3, '0'),
    g.annee,
    g.id_formation,
    g.id_groupe
FROM groupes g
...
SELECT 
    ...
    CASE d.nom
        ...
        ELSE 'GEN'
    END || '_' || LPAD(FLOOR(1 + RANDOM() * 20)::TEXT, 2, '0') as specialite,
    d.id_dept
FROM departements d
CROSS JOIN generate_series(1, 34) as n
ORDER BY RANDOM();
...
-- ============================================
-- 8. EXAMENS (One exam per module, 90 minutes)
-- ============================================
...
SELECT 
    'Departments' as entity, COUNT(*) as count FROM departements
UNION ALL
...
UNION ALL
SELECT 'Lieux', COUNT(*) FROM lieux_examen
UNION ALL
...
3. Padding:
   MySQL:  LPAD(n, 2, '0')
   PostgreSQL: LPAD(n::TEXT, 2, '0')  -- Need to cast to TEXT

4. Generate Series:
   MySQL:  Need to create with UNION
   PostgreSQL: generate_series(1, 10) -- Built-in function

5. Arrays:
   MySQL:  Not supported
   PostgreSQL: ARRAY['L1', 'L2', 'L3'] with unnest()

6. Type Casting:
   MySQL:  Implicit in most cases
   PostgreSQL: Use ::INTEGER or CAST(x AS INTEGER)

7. String Functions:
   MySQL:  FLOOR(RAND() * 1000)
   PostgreSQL: FLOOR(RANDOM() * 1000)::INTEGER
*/
