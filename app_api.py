...
def sanitize_dataframe(df):
    """Replace pandas NaT and NaN with None for JSON serialization"""
    return df.replace({pd.NaT: None, pd.NA: None, float('nan'): None})
...
def departements():
    try:
        ...
        print(f"Error in departements: {e}")
...
def annees():
    ...
    try:
        ...
        print(f"Error in annees: {e}")
        return fail(str(e), 500)

@app.get("/api/periodes")
def periodes():
    """Get all periods - FIXED to handle NULL dates"""
    try:
        conn = get_conn()
        cur = conn.cursor()
        ...
        for row in results:
            periods.append({
                "id_periode": row[0],
                "description": row[1],
                "date_debut": row[2].isoformat() if row[2] else None,
                "date_fin": row[3].isoformat() if row[3] else None,
                "generation_time_seconds": row[4],
                "generation_completed_at": row[5].isoformat() if row[5] else None,
                "has_planning": row[6]
            })
...
def schedule():
    """Get exam schedule - FIXED KeyError for MergedGroups"""
    formation_id = request.args.get("formation_id", type=int)
    annee = request.args.get("annee", type=str)
    ...
    try:
        ...
        for r in rows:
            ...
            # Avoid duplicates for merged exams
            exam_signature = (
                r.get("Module"),
                r.get("exam_date"),
                r.get("Start"),
            ...
            if exam_signature not in existing:
                groups[key]["exams"].append({
                    "date": str(r.get("exam_date")) if r.get("exam_date") else None,
...
def professeurs():
    try:
        ...
        print(f"Error in professeurs: {e}")
        return fail(str(e), 500)
...
def prof_schedule():
    ...
    try:
        ...
        print(f"Error in prof_schedule: {e}")
        return fail(str(e), 500)
...
def room_conflicts(pid: int):
    try:
        ...
        print(f"Error in room_conflicts: {e}")
        return fail(str(e), 500)
...
def student_schedule():
    ...
    try:
        ...
        for r in rows:
            ...
            if split_part:
                r["FullGroupLabel"] = f"{group_code} (Part {split_part})"
            else:
                r["FullGroupLabel"] = group_code

        return ok(rows)
    except Exception as e:
        print(f"Error in student_schedule: {e}")
        return fail(str(e), 500)
...
def dash_room_dist():
    ...
    try:
        ...
        print(f"Error in dash_room_dist: {e}")
        return fail(str(e), 500)
...
def dash_top_rooms():
    ...
    try:
        ...
        print(f"Error in dash_top_rooms: {e}")
        return fail(str(e), 500)
...
def dash_prof_load():
    ...
    try:
        ...
        print(f"Error in dash_prof_load: {e}")
        return fail(str(e), 500)
...
def dash_prof_conflicts():
    ...
    try:
        ...
        print(f"Error in dash_prof_conflicts: {e}")
        return fail(str(e), 500)
